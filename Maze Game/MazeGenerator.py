...
# initalise Pygame
pygame.init() # initiliases pygame modules so errors appear could do seperatley
pygame.mixer.init() # initilises sound
screen = pygame.display.set_mode((WIDTH, HEIGHT)) # sets screen size
pygame.display.set_caption("Python Maze Generator") # set caption of screen
clock = pygame.time.Clock() # creates a clock object that has many methods like .tick
...
def push_up(x, y):
    ...
    pygame.display.update()                                               # to animate the wall being removed
                                                                 #.rect(surface, color, (x pos, y pos ,width, height))
